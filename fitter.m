function [pd,stat,D] = fitter(data,varargin)
%% ==== fitter ====
...
% varargin
okargs =   {'sortby' 'dist' 'verbosity' 'plotpdf' 'discrete'};
defaults = { 'adstat'   []   1          0         0};
[sortby,dist,verbosity,plotpdf,discrete] = internal.stats.parseArgs(okargs,defaults,varargin{:});
sortby=lower(sortby);
...
for i = 1:length(PDs)
    ...
    x_sorted = sort(data);
    cdf_vals = cdf(PDs{i}, x_sorted);
    valid = isfinite(x_sorted) & isfinite(cdf_vals);
    if sum(valid) >= 1
        [~,pval,ks] = kstest(data,'cdf',[x_sorted(valid), cdf_vals(valid)]);
    else
        ks = NaN; pval = NaN;
    end
    D(i).KSstat = ks;
...
indx1=1:length(D);
[~,sortin]=ismember(sortby,{'adstat','pval','aic','nosort','kspval'});
if sortin == 1
    [~,indx1]=sort([D.ADstat]);
...
if plotpdf
    figure()
    hh = histogram(data,'Normalization','pdf','NumBins',10);
